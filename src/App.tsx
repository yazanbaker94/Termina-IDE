...
const App: React.FC = () => {
  ...
  const appendTerminalBuffer = useCallback((sessionId: string, data: string) => {
    setSessionRuntime((prev) => {
      ...
      if (buf.length > MAX_TERMINAL_BUFFER) buf = buf.slice(buf.length - MAX_TERMINAL_BUFFER);
  ...
  const refreshGitStatus = useCallback(async () => {
    try {
      ...
      setGitStatus(status);
    } catch (err) { console.error('Failed to refresh git status:', err); }
  }, []);
          ...
          if (dirty) {
            ...
            try {
              if (currentFile.language === 'image') {
                const asset = await window.electronAPI.getAssetDataUrl(currentFile.path);
                setActiveFile({ name: currentFile.name, path: currentFile.path, content: asset.dataUrl, language: 'image' });
  ...
  const attachFsChangeListener = useCallback(() => {
    fsListenerRef.current?.();
    fsListenerRef.current = window.electronAPI.onFileChanged(handleFileChanged);
  }, [handleFileChanged]);
  ...
  const setupAgentListeners = useCallback(() => {
    ...
    const unsubData = window.electronAPI.onAgentData(({ sessionId, data }) => {
      ...
      if (xtermWriteRef.current.write && xtermWriteRef.current.sessionId === sessionId && activeSessionIdRef.current === sessionId) {
        xtermWriteRef.current.write(data);
      }
    });

    const unsubExit = window.electronAPI.onAgentExit(({ sessionId, exitCode }) => {
      if (exitCode >= 0) updateSessionRuntime(sessionId, { agentStatus: 'exited', exitCode });
    });
  ...
  const selectProjectCore = useCallback(async (project: StoredProject, targetSessionId?: string | null) => {
    try {
      ...
      const updated = { ...appData, activeProjectId: project.id, projects: appData.projects.map((p) => p.id === project.id ? { ...p, openedAt: Date.now() } : p) };
      persist(updated);
      ...
      if (targetSessionId && projectSessions.some((s) => s.id === targetSessionId)) {
        ...
        finalSessionId = (cached && projectSessions.some((s) => s.id === cached)) ? cached : projectSessions[projectSessions.length - 1].id;
      }
      setActiveSessionId(finalSessionId);
      if (finalSessionId) activeSessionByProjectIdRef.current[project.id] = finalSessionId;
  ...
  }, [appData, persist, refreshGitStatus, reconcileAgentStatus, attachFsChangeListener]);

  const handleSelectProject = useCallback(async (project: StoredProject) => {
  ...
  const handleSelectProjectSession = useCallback(async (project: StoredProject, sessionId: string) => {
    if (project.id !== activeProjectId) { await selectProjectCore(project, sessionId); return; }
  ...
  const handleSelectSession = useCallback((sessionId: string) => {
    ...
    if (selectedSession && activeProjectId) activeSessionByProjectIdRef.current[activeProjectId] = sessionId;
    ...
    if (nextRuntime?.activeFilePath) {
      window.electronAPI.readFile(nextRuntime.activeFilePath).then((result) => {
        setActiveFile({ name: nextRuntime.activeFileName ?? nextRuntime.activeFilePath!.split(/[\\/]/).pop() ?? 'untitled', path: result.filePath, content: result.content, language: result.language });
  ...
  const handleNewChat = useCallback(async () => {
    ...
    const prevId = activeSessionId;
    if (prevId) {
      updateSessionRuntime(prevId, { activeFilePath: activeFileRef.current?.path ?? null, activeFileName: activeFileRef.current?.name ?? null, diffPath: activeDiffRef.current?.filePath ?? null });
    }
    const sessionId = generateId();
    const projectChats = sessions.filter((s) => s.projectId === activeProjectId);
    const newSession: StoredSession = { id: sessionId, projectId: activeProjectId, rootPath: rootPath ?? '', label: `Chat ${projectChats.length + 1}`, createdAt: Date.now() };
    const newSessions = [...sessions, newSession];
    setSessions(newSessions);
    setActiveSessionId(sessionId);
    if (activeProjectId) activeSessionByProjectIdRef.current[activeProjectId] = sessionId;
  ...
  const handleOpenFolder = useCallback(async () => {
    try {
      ...
      if (projectSessions.length > 0) {
        ...
        const targetId = (cached && projectSessions.some((s) => s.id === cached)) ? cached : projectSessions[projectSessions.length - 1].id;
  ...
  }, [appData, persist, reconcileAgentStatus, attachFsChangeListener]);

  // Auto-reopen active project on startup — must also attach fs listener
  useEffect(() => {
    const reopenProject = async () => {
      ...
      try {
        const result = await window.electronAPI.openProjectPath(project.rootPath);
        if (!result.success || !result.rootPath) { setSaveStatus('Project folder not found'); setTimeout(() => setSaveStatus(''), 3000); return; }
        ...
        refreshGitStatus();
        attachFsChangeListener();
      } catch (err) { console.error('Failed to reopen project:', err); }
    };
    reopenProject();
  ...
  const handleRemoveProject = useCallback(async (projectId: string) => {
    ...
    for (const s of projectSessions) { try { await window.electronAPI.stopAgent(s.id); } catch {} }
    const cleanedRuntime = { ...sessionRuntimeRef.current };
    for (const s of projectSessions) delete cleanedRuntime[s.id];
  ...
  const handleWriteAgent = useCallback(async (sessionId: string, input: string) => {
    try {
      ...
      if (!result.success) { updateSessionRuntime(sessionId, { agentStatus: 'exited', error: 'Agent process is no longer running.' }); await reconcileAgentStatus(); }
